...
```
`Rack::Session::Cookie`基本实现了Cookie存储Session所需要的全部逻辑，包括各种编码方法，以及简单的SHA1验证。其中Rails并没有使用它的SHA1验证功能，这可能是因为Rails要求`CookieStore`使用Rails自己实现的`CookieJar`类，并且需要对付一些安全攻击。
