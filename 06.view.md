...
```
之前说过，`@responses`记录了Mime类型到用于response的block的Hash，这里将返回这个block。在生成Scaffold代码的时候，html通常没有block，而json一般会有一句`render`语句。如果没有block的话，将会执行`default_render`方法，我们这里解析这个方法，因为只运行`render`方法的情况相对更简单一些。