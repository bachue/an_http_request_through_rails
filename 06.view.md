...
```
这个module实际相当于一个抽象类，它提供了对`Mime::SET`，也就是Rails所有注册的Mime类型的同名方法集合，每个同名方法都是对`custom`方法的调用，而`custom`方法则在实际的类中实现，也就是这里`ActionController::MimeResponds::Collector`的实现：
