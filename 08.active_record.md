...
##### Mass Assignment Security, Validation, Transaction and Save #####

这里本篇最后一个用例，将解析ActiveRecord除了查询以外其他一些零碎的功能。
...
```
`@_start_transaction_state`当第一次执行save或destroy方法的时候被创建，用以记录当前Transaction的情况。然后这里主要是计算`@_start_transaction_state`的level并在需要的时候删除掉这个对象。
