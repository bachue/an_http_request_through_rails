...
，注意这个方法定义在`ActiveRecord::Associations::Association`模块中，注意区分这个模块和`Builder::Association`，这个文件直接定义在`activerecord-3.2.13/lib/active_record/associations`目录下的`association.rb`文件中：
